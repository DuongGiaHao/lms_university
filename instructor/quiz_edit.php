...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $db->prepare("UPDATE quizzes SET course_id = ?, title = ?, description = ?, time_limit = ?, max_attempts = ?, due_date = ? WHERE id = ?");
    if ($stmt->execute([$course_id, $title, $description, $time_limit, $max_attempts, $due_date, $id])) {
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <div class="d-flex justify-content-between align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2"><i class="fas fa-edit"></i> Edit Quiz</h1>
                </div>
                <?php if ($message): ?>
                    ...
                    <div class="card-body">
                        <form method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" name="time_limit" id="time_limit" class="form-control" value="<?= $quiz['time_limit'] ?>" min="1" required>
                            </div>
                            <div class="mb-3">
                                <label for="max_attempts" class="form-label">Max Attempts</label>
                                <input type="number" name="max_attempts" id="max_attempts" class="form-control" value="<?= $quiz['max_attempts'] ?>" min="1" required>
                            </div>
                            <div class="mb-3">
                                <label for="due_date" class="form-label">Due Date</label>
                                <input type="datetime-local" name="due_date" id="due_date" class="form-control" value="<?= date('Y-m-d\TH:i', strtotime($quiz['due_date'])) ?>" required>
                            </div>
